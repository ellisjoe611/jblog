			...
			<div id="content" class="full-screen">
				...
				<form action="${pageContext.request.contextPath}/${blogVo.user_id}/admin/write" method="post">
			      		...
			      		<tr>
			      			...
			      			<td>
				      			...
				      			<select name="category_no">
				      				<c:forEach items='${categoryScrollList }' var='categoryScrollVo' varStatus='status'>
				      					<option value="${categoryScrollVo.no }">${categoryScrollVo.name }</option>
				      				</c:forEach>
				      			</select>
				      		</td>
			      		</tr>
			      	...
			      	<c:if test="${not empty previousVo }">
      					<p>포스트 추가에 실패했습니다. 누락 여부 등을 다시 확인해주세요!</p>
      				</c:if>
				</form>
			</div>
		</div>
