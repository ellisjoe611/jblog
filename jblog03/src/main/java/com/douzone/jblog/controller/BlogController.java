...
public class BlogController {
	...
	public String displayAll(
			...
			@PathVariable("id") String blogOwnerId,
			@PathVariable Optional<Long> categoty,
		...
		// 3. user_id가 가지고 있는 포스트 목록 출력 ( [category_no] >>> list [no, title, reg_date(년/월/일 까지만), category_no] )
		// 만약 postNo가 없으면...?
	...
	public String adminCategoryRemove(
			@AuthUser UserVO authUser,
			@PathVariable("id") String blogOwnerId,
			@PathVariable("no") Long category_no,
			Model model) {
	...
	public String adminPost(
		...
		// 카테고리 스크롤를 받아서 넘긴다.
		model.addAttribute("categoryScrollList", (List<CategoryVO>) categoryService.getCategoryScroll(authUser.getId()));
